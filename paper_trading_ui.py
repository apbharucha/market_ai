    ...
    def _chart_fragment():
        ...
        if chart_settings.get("show_model_trades", False) and model_markers:
            ...
            if summary["total_trades"] > 0:
                ...
                with sc1: st.metric("Trades", summary["total_trades"])
                with sc2: st.metric("Win Rate", f"{summary['win_rate']:.0%}")
                with sc3: st.metric("Avg Win", f"{summary['avg_win']:+.2%}")
                with sc4: st.metric("Avg Loss", f"{summary['avg_loss']:+.2%}")
                with sc5: st.metric("Total Return", f"{summary['total_return']:+.2%}")
                sc6, sc7, sc8 = st.columns(3)
                with sc6: st.metric("Best Trade", f"{summary['best_trade']:+.2%}")
                with sc7: st.metric("Worst Trade", f"{summary['worst_trade']:+.2%}")
                with sc8:
